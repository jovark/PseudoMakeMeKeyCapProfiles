...
// translate([17.7, 0, 0])
// import("ldsa/LDSA_1u_thumb.stl");
// import("ldsa/LDSA_Thumb_1.5u.stl");
// import("ldsa/LSDA_1u_homing.stl");
...
module keycap_with_support_ldsa(type = "alpha") {
  radious = 0.75;
  size = -16;
  ...
  if (type == "homing") {
    import("ldsa/LDSA_Homing_1u.stl");
  }
  else if (type == "thumb") {
...
module generate_ldsa_keycaps(type = "alpha", row) {
  offset_x = 18;
  offset_y = 17;

  for ( j = [ 0 : 1 : 3 - 1]) {
    for ( i = [ 0 : 1 : 3 - 1]) {
      translate([offset_x * i, offset_y * j, 0]) {
        if (i == 0 && j == 0) {
          translate([offset_x, -offset_y, 0]){
            translate([0,((+16.0) / 2 - 0.75 * 2 + 0.045), -0.75 * 2])
              ...
              cylinder(h=17, r=0.75);
            rotate([0, 0, 180])
              keycap_with_support_ldsa(type);
          }
        }
        keycap_with_support_ldsa(type);
      }
    }
    translate([0, offset_y * j, -0.75 ])
      translate([-0.75,((-16.0) / 2 + 0.75 - 0.045), -0.75])
      color("red")
      rotate([180,-90,0])
      cylinder(h=(offset_x + 0.75) * 2, r=0.75);
  }
  translate([offset_x,((-16.0) / 2 + 0.75 - 0.045), -0.75 * 2])
    color("red")
    rotate([-90,90,0])
    cylinder(h=offset_y * 2, r=0.75);
}
